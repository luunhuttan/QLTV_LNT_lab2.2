...
app.get('/api/members', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM members ORDER BY member_id DESC');
    res.json(rows);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/return', async (req, res) => {
  ...
  if (Number.isNaN(id)) return res.status(400).json({ error: 'Invalid borrowing_id' });
  const conn = await pool.getConnection();
  try {
    await conn.beginTransaction();
    const [[bor]] = await conn.query(
      'SELECT book_id, return_date FROM borrowing WHERE borrowing_id = ? FOR UPDATE',
      [id]
    );
    if (!bor) {
      await conn.rollback();
    ...
    await conn.execute('UPDATE borrowing SET return_date = CURRENT_DATE() WHERE borrowing_id = ?', [id]);
